...
export const AGENT_IMPLEMENTATION_EFFORTS = ["quick", "standard", "thorough"] as const;
export type AgentImplementationEffort = (typeof AGENT_IMPLEMENTATION_EFFORTS)[number];

export function implementationThinkingLevel(effort: unknown): ThinkingLevel {
...
export function buildImplementationSystemPrompt(timeoutMs: number): string {
  const timeoutMinutes = Math.round(timeoutMs / 60_000);

  return `You are a bounded implementation subagent operating inside a coding assistant.
...
- Avoid parallel write-heavy work. This agent is intended to be the only mutating subagent for the current task.
...
export function buildImplementationUserPrompt(params: Record<string, unknown>): string {
