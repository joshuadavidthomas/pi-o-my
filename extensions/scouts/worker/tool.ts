...
export const WORKER_TOOL: ToolDefinition<typeof WorkerParams, ScoutDetails> = {
  ...
  label: "Worker",
  description:
    "Bounded implementation worker. Use worker only after the main agent has decided what to change and can provide a concrete implementation brief. Worker can read, edit, write, run commands, and verify, but it should not do open-ended discovery or architecture planning. Use finder/oracle/librarian before worker when the target or design is unclear. Do not launch parallel workers unless future tooling explicitly supports disjoint write scopes; this worker enforces one active worker at a time. Usually omit the optional `model` parameter unless the user explicitly asked for a specific model/provider.",
